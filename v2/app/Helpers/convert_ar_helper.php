...
{
	$cnv = new convert_ar($num, 'male');
	if ($num == 0) return 'صفر فقط';
	return $cnv->convert_number();
}
function ctm_convert($num, $gender)
{
	$cnv = new convert_ar($num, $gender);
	return $cnv->convert_number();
}

function to_ar_num($num)
{
			...
			case '3': {
					if (isset($words[$mf[$place]][$number])) {
						...
						if ($twoyony == 2) {
							...
							if ($twoyony != '') $return = $return . ' و' . $twoyony;
							switch ($place) {
								case '2': {
										$return .= ' ألف';
									}
