...
class SimpleXLSXEx
{
    ...
    public function readFonts()
    {
        ...
        if (isset($this->xlsx->styles->fonts->font)) {
            foreach ($this->xlsx->styles->fonts->font as $v) {
                ...
                $f = [
                    "b" =>
                    isset($v->b) && ($v->b["val"] === null || $v->b["val"]),
                    "i" =>
                    isset($v->i) && ($v->i["val"] === null || $v->i["val"]),
                    "u" => $u,
                    "strike" =>
                    isset($v->strike) &&
                        ($v->strike["val"] === null || $v->strike["val"]),
                    "sz" => isset($v->sz["val"]) ? (int) $v->sz["val"] : 11,
                    "color" => $this->getColorValue($v->color),
                    "name" => isset($v->name["val"])
                        ? (string) $v->name["val"]
                        : "Calibri",
                    "family" => isset($v->family["val"])
                        ? (int) $v->family["val"]
                        : 2,
                    "charset" => isset($v->charset["val"])
                        ? (int) $v->charset["val"]
                        : 1,
                ...
                if (isset($v->patternFill)) {
                    $this->fills[] = [
                        "pattern" => isset($v->patternFill["patternType"])
                            ? (string) $v->patternFill["patternType"]
                            : "none",
                        "fgcolor" => $this->getColorValue(
                            $v->patternFill->fgColor
                        ),
                        "bgcolor" => $this->getColorValue(
                            $v->patternFill->bgColor
                        ),
                    ];
                }
            }
    ...
    public function readXfs()
    {
        ...
        if (isset($this->xlsx->styles->cellXfs->xf)) {
            ...
            foreach ($this->xlsx->styles->cellXfs->xf as $v) {
                ...
                // fill
                $cf["bgcolor"] = isset($cf["fillId"])
                    ? $this->fills[$cf["fillId"]]["fgcolor"]
                    : null;
                ...
                if ($cf["b-right-style"]) {
                    ...
                    $css .=
                        "border-right-color: #" . $cf["b-right-color"] . ";";
                    $css .= "border-right-width: thin;";
                }
                if ($cf["b-bottom-style"]) {
                    $css .=
                        "border-bottom-style: " . $cf["b-bottom-style"] . ";";
                    $css .=
                        "border-bottom-color: #" . $cf["b-bottom-color"] . ";";
    ...
    protected function valueEx($data, $x = null, $y = null)
    {
        ...
        if (
            $r["value"] !== "" &&
            !$st["align"] &&
            !in_array($r["type"], ["s", "str", "inlineStr", "e"], true)
        ) {
    ...
    public function getColorValue(SimpleXMLElement $a = null, $default = "")
    {
        ...
        if ($a["rgb"] !== null) {
            ...
            $a["indexed"] !== null &&
            isset(static::$IC[(int) $a["indexed"]])
        ) {
            $c = static::$IC[(int) $a["indexed"]];
        } elseif (
            $a["theme"] !== null &&
            isset($this->themeColors[(int) $a["theme"]])
        ) {
