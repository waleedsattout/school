    ...
    protected function _unzip($filename, $is_data = false)
    {
        ...
        */
        ...
        foreach ($aE as $vZ) {
            ...
            $aP = unpack(
                "v1VN/v1GPF/v1CM/v1FT/v1FD/V1CRC/V1CS/V1UCS/v1FNL/v1EFL",
                $vZ
            );
...
*/
    ...
    protected function _substr($str, $start, $length = null)
    {
        return ini_get("mbstring.func_overload") & 2
            ? mb_substr(
                $str,
                $start,
                $length === null ? mb_strlen($str, "8bit") : $length,
                "8bit"
            )
        ...
        if ($relations = $this->getEntryXML("_rels/.rels")) {
            foreach ($relations->Relationship as $rel) {
                ...
                if (
                    ...
                    if (
                        $workbookRelations = $this->getEntryXML(
                            dirname($rel_target) . "/_rels/workbook.xml.rels"
                        )
                    ) {
                        // Loop relations for workbook and extract sheets...
                        foreach ($workbookRelations->Relationship
                            as $workbookRelation) {
                            $wrel_type = basename(
                                trim((string) $workbookRelation["Type"])
                            ); // worksheet
                            $wrel_path = $this->_getTarget(
                                dirname($rel_target),
                            ...
                            if ($wrel_type === "worksheet") {
                                ...
                                if (
                                    $sharedStrings = $this->getEntryXML(
                                        $wrel_path
                                    )
                                ) {
                                    foreach ($sharedStrings->si as $val) {
                                        if (isset($val->t)) {
                                            ...
                                            $this->sharedstrings[] = $this->_parseRichText(
                                                $val
                                            );
                                        }
                                    }
                                ...
                                if (isset($this->styles->numFmts->numFmt)) {
                                    foreach ($this->styles->numFmts->numFmt
                                        as $v) {
                                        $this->nf[(int) $v["numFmtId"]] =
                                            (string) $v["formatCode"];
                                    }
                                }
                                ...
                                if (isset($this->styles->cellXfs->xf)) {
                                    foreach ($this->styles->cellXfs->xf as $v) {
                                        ...
                                        foreach ($v->attributes()
                                            as $k1 => $v1) {
                                            $x[$k1] = (int) $v1;
                                        }
                                        if (isset($x["numFmtId"])) {
                                            if (
                                                isset($this->nf[$x["numFmtId"]])
                                            ) {
                                                $x["format"] =
                                                    $this->nf[$x["numFmtId"]];
                                            } elseif (
                                                isset(self::$CF[$x["numFmtId"]])
                                            ) {
                                                $x["format"] =
                                                    self::$CF[$x["numFmtId"]];
                                            }
    ...
    public function getEntryXML($name)
    {
        if ($entry_xml = $this->getEntryData($name)) {
            ...
            $entry_xml = preg_replace(
                ...
                '<$1>',
                $entry_xml
            ); // fix namespaced openned tags
            $entry_xml .= " ";
            $entry_xml = preg_replace(
                "/<\/[a-zA-Z0-9]+:([^>]+)>/",
                '</$1>',
                $entry_xml
            ); // fix namespaced closed tags
            $entry_xml .= " ";
            ...
            $entry_xmlobj = simplexml_load_string(
                $entry_xml,
                "SimpleXMLElement",
    ...
    public function getEntryData($name)
    {
        ...
        foreach ($this->package["entries"] as &$entry) {
            if (
                $this->_strtoupper($entry["path"]) === $dir &&
                $this->_strtoupper($entry["name"]) === $name
            ) {
                if ($entry["error"]) {
                    return false;
                }
                ...
                if (!$entry["error"] && $entry["cm"] > -1) {
                    ...
                    if ($entry["data"] === false) {
                        ...
                        $this->_strlen($entry["data"]) !== (int) $entry["ucs"]
                    ) {
                        $entry["error"] = 3;
                        $entry["error_msg"] =
    ...
    public function worksheet($worksheetIndex = 0)
    {
        if (isset($this->sheets[$worksheetIndex])) {
            ...
            if (
                !isset($this->hyperlinks[$worksheetIndex]) &&
                isset($ws->hyperlinks)
            ) {
                ...
                $sheet_rels =
                    str_replace(
                        "worksheets",
                        "worksheets/_rels",
                        $this->sheetFiles[$worksheetIndex]
                    ) . ".rels";
                $link_ids = [];
    ...
    {
        return ini_get("mbstring.func_overload") & 2
            ? mb_strpos($haystack, $needle, $offset, "8bit")
    ...
    {
        $ref = (string) $cell["r"];
        return isset($this->hyperlinks[$worksheetIndex][$ref])
            ? $this->hyperlinks[$worksheetIndex][$ref]
            : "";
    }
    ...
    public function toHTML($worksheetIndex = 0)
    {
        ...
        foreach ($this->readRows($worksheetIndex) as $r) {
            ...
            foreach ($r as $c) {
                $s .=
                    "<td nowrap>" .
                    ($c === "" ? "&nbsp" : htmlspecialchars($c, ENT_QUOTES)) .
                    "</td>";
            }
            $s .= "</tr>\r\n";
        }
        ...
        foreach ($this->readRowsEx($worksheetIndex) as $r) {
            ...
            foreach ($r as $c) {
                ...
                $s .=
                    ...
                    ($c["value"] === ""
                        ? "&nbsp"
                        : htmlspecialchars($c["value"], ENT_QUOTES)) .
                    "</" .
                    $tag .
                    ">";
                $x++;
    ...
    public function rows($worksheetIndex = 0, $limit = 0)
    {
        return iterator_to_array(
            $this->readRows($worksheetIndex, $limit),
            false
        );
    }
    // thx Gonzo
    ...
    {
        return iterator_to_array(
            $this->readRowsEx($worksheetIndex, $limit),
            false
        );
    }
    // https://github.com/shuchkin/simplexlsx#gets-extend-cell-info-by--rowsex
